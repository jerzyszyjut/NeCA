...
class ConeGeometry(object):
    ...
    def __init__(self, data, index=0):
        ...
        # -------------------------------------------------------------------------------------
        self.DSD = (
            data["projections_settings"][index]["sid"] / 1000
        )  # Distance Source Detector      (m)
        self.DSO = (
            data["projections_settings"][index]["sod"] / 1000
        )  # Distance Source Origin        (m)
        # Detector parameters
        self.nDetector = np.array(
            data["global"]["image_resolution"]
        )  # number of pixels              (px)
        self.dDetector = (
            np.array(data["global"]["image_spacing"]) / 1000
...
class TIGREDataset(Dataset):
    ...
    def __init__(self, data, device="cuda"):
        ...
        self.projs = torch.tensor(
            data["projections"], dtype=torch.float32, device=device
        )
        ...
        coords = torch.stack(
            torch.meshgrid(
                torch.linspace(
                    0, geo.nDetector[1] - 1, geo.nDetector[1], device=device
                ),
                torch.linspace(
                    0, geo.nDetector[0] - 1, geo.nDetector[0], device=device
                ),
            ),
            -1,
        )
        ...
        self.voxels = torch.tensor(
            self.get_voxels(geo), dtype=torch.float32, device=device
        )
